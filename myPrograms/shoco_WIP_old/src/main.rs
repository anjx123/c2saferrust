use core::arch::x86_64::{
    __m128i, _mm_setzero_si128, _mm_load_si128, _mm_cmpgt_epi16, _mm_and_si128, _mm_movemask_epi8,
};
...
pub fn check_indices(indices: &[i16], pack_n: i32) -> i32 {
    // Ensure the slice has at least 8 elements required for a __m128i load.
    assert!(
        indices.len() >= 8,
        "indices slice must contain at least 8 elements"
    );

    // Safety:
    // 1. The indices slice is assumed to be properly aligned for _mm_load_si128.
    // 2. The global packs[pack_n as usize].masks is assumed to be properly aligned and valid.
    unsafe {
        let zero: __m128i = _mm_setzero_si128();
        let indis: __m128i = _mm_load_si128(indices.as_ptr() as *const __m128i);
        let masks: __m128i =
            _mm_load_si128(packs[pack_n as usize].masks.as_ptr() as *const __m128i);
        let cmp_indices: __m128i = _mm_cmpgt_epi16(indis, masks);
        let cmp_masks: __m128i = _mm_cmpgt_epi16(masks, zero);
        let cmp: __m128i = _mm_and_si128(cmp_indices, cmp_masks);
        let result: i32 = _mm_movemask_epi8(cmp);
