...
#[repr(C)]
pub struct genann {
    pub inputs: libc::c_long,
    pub hidden_layers: libc::c_long,
    pub hidden: libc::c_long,
    pub outputs: libc::c_long,
    pub activation_hidden: genann_actfun,
    pub activation_output: genann_actfun,
    pub total_weights: libc::c_long,
    pub total_neurons: libc::c_long,
...
pub unsafe extern "C" fn genann_init_sigmoid_lookup(mut ann: *const genann) {
    let f = (sigmoid_dom_max - sigmoid_dom_min) / 4096 as libc::c_int as libc::c_double;
...
pub unsafe extern "C" fn genann_act_sigmoid_cached(
    ...
    if a.is_nan() as i32 == 0 {} else {
        __assert_fail(
            ...
            b"/workspace/programs/genann/genann.c\0" as *const u8 as *const libc::c_char,
    ...
    'c_905: {
        if a.is_nan() as i32 == 0 {} else {
            __assert_fail(
                b"!isnan(a)\0" as *const u8 as *const libc::c_char,
                b"/workspace/programs/genann/genann.c\0" as *const u8
    ...
    let mut j = ((a - sigmoid_dom_min) * interval + 0.5f64) as size_t;
    if (j >= 4096 as libc::c_int as libc::c_ulong) as libc::c_int as libc::c_long != 0 {
        return lookup[(4096 as libc::c_int - 1 as libc::c_int) as usize];
    }
...
pub unsafe extern "C" fn genann_init(
    ...
    let size = (::core::mem::size_of::<genann>() as libc::c_ulong)
        .wrapping_add(
    ...
    (*ret).inputs = inputs as libc::c_long;
    (*ret).hidden_layers = hidden_layers as libc::c_long;
    (*ret).hidden = hidden as libc::c_long;
    (*ret).outputs = outputs as libc::c_long;
    (*ret).total_weights = total_weights as libc::c_long;
    (*ret).total_neurons = total_neurons as libc::c_long;
    (*ret)
...
pub unsafe extern "C" fn genann_read(mut in_0: *mut FILE) -> *mut genann {
    ...
    *__errno_location() = 0 as libc::c_int;
    ...
    let mut i: libc::c_int = 0;
    i = 0 as libc::c_int;
    while (i as libc::c_long) < (*ann).total_weights {
        *__errno_location() = 0 as libc::c_int;
        rc = fscanf(
            in_0,
...
pub unsafe extern "C" fn genann_run(
    ...
    if (*ann).hidden_layers == 0 {
        let mut ret = o;
        j = 0 as libc::c_int;
        while (j as libc::c_long) < (*ann).outputs {
            let fresh0 = w;
            w = w.offset(1);
            let mut sum = *fresh0 * -1.0f64;
            k = 0 as libc::c_int;
            while (k as libc::c_long) < (*ann).inputs {
                let fresh1 = w;
                w = w.offset(1);
    ...
    j = 0 as libc::c_int;
    while (j as libc::c_long) < (*ann).hidden {
        let fresh3 = w;
        w = w.offset(1);
        let mut sum_0 = *fresh3 * -1.0f64;
        k = 0 as libc::c_int;
        while (k as libc::c_long) < (*ann).inputs {
            let fresh4 = w;
            w = w.offset(1);
    ...
    h = 1 as libc::c_int;
    while (h as libc::c_long) < (*ann).hidden_layers {
        j = 0 as libc::c_int;
        while (j as libc::c_long) < (*ann).hidden {
            let fresh6 = w;
            w = w.offset(1);
            let mut sum_1 = *fresh6 * -1.0f64;
            k = 0 as libc::c_int;
            while (k as libc::c_long) < (*ann).hidden {
                let fresh7 = w;
                w = w.offset(1);
    ...
    if w.offset_from((*ann).weight) as libc::c_long == (*ann).total_weights {} else {
        __assert_fail(
    ...
    if o.offset_from((*ann).output) as libc::c_long == (*ann).total_neurons {} else {
        __assert_fail(
...
pub unsafe extern "C" fn genann_train(
    ...
    h = ((*ann).hidden_layers - 1 as libc::c_int as libc::c_long) as libc::c_int;
    while h >= 0 as libc::c_int {
        let mut o_0: *const libc::c_double = ((*ann).output)
            .offset((*ann).inputs as isize)
            .offset((h as libc::c_long * (*ann).hidden) as isize);
        let mut d_0 = ((*ann).delta)
            .offset((h as libc::c_long * (*ann).hidden) as isize);
        let dd: *const libc::c_double = ((*ann).delta)
            .offset(((h + 1 as libc::c_int) as libc::c_long * (*ann).hidden) as isize);
        let ww: *const libc::c_double = ((*ann).weight)
            .offset(
                (((*ann).inputs + 1 as libc::c_int as libc::c_long) * (*ann).hidden)
                    as isize,
            )
            .offset(
                (((*ann).hidden + 1 as libc::c_int as libc::c_long) * (*ann).hidden
                    * h as libc::c_long) as isize,
            );
        j = 0 as libc::c_int;
        while (j as libc::c_long) < (*ann).hidden {
            ...
            while (k as libc::c_long)
                < (if h as libc::c_long
                    == (*ann).hidden_layers - 1 as libc::c_int as libc::c_long
                {
                    (*ann).outputs
                } else {
                    (*ann).hidden
                })
            {
                ...
                let windex = (k as libc::c_long
                    * ((*ann).hidden + 1 as libc::c_int as libc::c_long)
                    + (j + 1 as libc::c_int) as libc::c_long) as libc::c_int;
    ...
    while h >= 0 as libc::c_int {
        ...
        let mut i_0: *const libc::c_double = ((*ann).output)
            .offset(
                (if h != 0 {
                    (*ann).inputs
                        + (*ann).hidden * (h - 1 as libc::c_int) as libc::c_long
                } else {
                    0 as libc::c_int as libc::c_long
                }) as isize,
            );
